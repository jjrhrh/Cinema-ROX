export default async function handler(req, res) {
  ...
  /* ── 1. بحث بدون Persisted Query (query مباشر) ── */
  let showId = null;
  try {
    ...
    if (hit) {
      showId = hit._id;
      console.log(`[stream] ✅ showId: ${showId} — ${hit.englishName || hit.name}`);
    }
  } catch(e) {
    console.log('[stream] فشل البحث:', e.message);
  }
  ...
  try {
    ...
    console.log(`[stream] rawSources: ${rawSources.length}`);
    ...
    console.log(`[stream] ✅ ${sources.length} مصدر`);
    return res.status(200).json({ sources, showId });

  } catch(e) {
    console.log('[stream] ❌ خطأ:', e.message);
