(function() {
  ...
  overlay.innerHTML = `
    <div class="rox-login-bg">
      <div class="rox-particles" id="roxParticles"></div>
      <div class="rox-login-card">
        <div class="rox-login-logo">Cinema <span>ROX</span></div>
        <div class="rox-login-tagline">عالمك السينمائي الخاص</div>
        <div class="rox-login-icon">🎬</div>
        <p class="rox-login-desc">أدخل كلمة المرور للوصول</p>
        <div class="rox-input-wrap">
          <input id="passInput" type="password" placeholder="••••••••••••••"
            class="rox-pass-input">
          <button class="rox-eye-btn" onclick="togglePassVis()">👁</button>
        </div>
        <button class="rox-enter-btn" onclick="checkPass()">
          <span>دخول</span>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
        </button>
        <p id="passErr" class="rox-pass-err">❌ كلمة المرور غير صحيحة</p>
      </div>
    </div>
  `;
  ...
  style.textContent = `
    .rox-login-bg {
      ...
      background: radial-gradient(ellipse at 60% 40%, #1a0000 0%, #000 70%);
      z-index:99999;
    ...
    .rox-login-logo {
      font-size:1.8rem;font-weight:900;color:#e50914;
      margin-bottom:4px;letter-spacing:1px;
    }
    .rox-login-logo span { color:#fff; }
    .rox-login-tagline {
      font-size:0.78rem;color:#666;margin-bottom:20px;
    }
    .rox-login-icon {
    ...
    .rox-login-desc {
      color:#888;font-size:0.88rem;margin-bottom:20px;
    }
    .rox-input-wrap {
    ...
    .rox-pass-input {
      ...
      font-size:1.1rem;font-family:Cairo,sans-serif;
      text-align:center;outline:none;letter-spacing:4px;
      box-sizing:border-box;
      transition:border-color 0.3s,box-shadow 0.3s;
    }
    .rox-pass-input:focus {
    ...
    .rox-enter-btn:active { transform:translateY(0); }
    .rox-pass-err {
      color:#e50914;font-size:0.85rem;
  ...
  // جسيمات متطايرة
  const pc = document.getElementById('roxParticles');
  for (let i = 0; i < 30; i++) {
    const p = document.createElement('div');
    p.className = 'rox-particle';
    p.style.cssText = `--x:${Math.random()*100}%;--d:${5+Math.random()*10}s;animation-delay:${Math.random()*8}s;width:${1+Math.random()*3}px;height:${1+Math.random()*3}px;`;
    pc.appendChild(p);
  }
  ...
  window.checkPass = function() {
    ...
    if (allowed.includes(val)) {
      ...
      if (err) { err.style.display = 'block'; err.style.animation = 'none'; setTimeout(() => err.style.animation = '', 10); }
      const inp = document.getElementById('passInput');
